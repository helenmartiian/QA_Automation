...
public class Validation {
    ...
    public static int countDigits(long number) {
        ...
        int count=1; // колличество знаков, 1 потому что нет чисел состоящих из 0 цифр
        while( number >= 10 ){ // проверяем, что кол-во знаков больше одного
            number = number/10;
            count++;
    ...
    public static long eval(long[] creditCardDigits) {
        ...
        long multiply = 1L; // отвечает за десятки
        for (int i=creditCardDigits.length-1;i>=0;i--){ // length-1, чтобы получить правильный индекс последнего элемента
            creditCardNumber=creditCardNumber+creditCardDigits[i]*multiply;
            multiply*=10;
        }
    ...
    /**
     * Превращает перевернуты массив цифр в число.
    ...
    public static long[] doubleSecond(long[] creditCardDigits) {
        for (int i = 0; i < creditCardDigits.length; i++){
            if (i%2!=0){ // каждый четный єлемент в жизни, єто нечетный индекс в джаве
    ...
    public static boolean isValid(long creditCard) {
       long[] creditCardReverse=toDigitsReverse(creditCard);
       creditCardReverse= doubleSecond(creditCardReverse);
       long sumDigits=sumDigits(creditCardReverse);
       boolean isValidCard =sumDigits%10 == 0;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(numValid(creditCards));
    ...
    public static void test22() {
        ...
        assert valid == 94 : "Test22 is not passed. Expected 94 but got " + valid;
    }
